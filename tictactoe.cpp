...
using namespace std;
 ...
 int team_wins(char ch1, char ch2){
  // Check diagonals
  if (check3(ch1, ch2, B[0][0], B[1][1], B[2][2]) == true) return 1;
  if (check3(ch1, ch2, B[0][2], B[1][1], B[2][0]) == true) return 1;
 
  // Check rows and columns
  for (int i=0; i<3; i++) {
    if (check3(ch1, ch2, B[0][i], B[1][i], B[2][i]) == true) return 1;
    if (check3(ch1, ch2, B[i][0], B[i][1], B[i][2]) == true) return 1;
  }
